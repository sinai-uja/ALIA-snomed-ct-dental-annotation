...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        default=os.path.join(project_root, "config", "config_e2e.yaml"),
        help="Ruta del archivo YAML de configuración (default: config_e2e.yaml para NER simple)",
    )
    parser.add_argument(
        "--base-dir",
        default=None,
    ...
    base_dir = os.path.abspath(args.base_dir) if args.base_dir else project_root
    config_path = (
    ...
    # Resolver rutas relativas respecto al directorio del YAML,
    # no respecto al cwd de ejecución.
    cfg = load_config(config_path, base_dir=os.path.dirname(config_path))
    run_evaluation(cfg)